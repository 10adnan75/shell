 ...
 * CommandHandler.java
 *
 * <p>Handles parsing and execution of shell commands, including builtins, external commands,
 * pipelines, and redirection. Maintains the current working directory and command history.
 *
...
/**
 * Central class responsible for parsing user input, handling command execution (builtins and
...
public class CommandHandler {
  ...
  /**
   * Parses and executes a single shell command or pipeline, handling redirection and updating the
   * current directory as needed.
   *
   * @param input The command line input string.
   * @param currentDirectory The current working directory.
   * @return The updated current directory after command execution.
   */
  public Path handleCommand(String input, Path currentDirectory) {
    ...
    Streams streams = extractResult.streams;
    if (partsList.isEmpty()) return this.currentDirectory;
    ...
    if (builtinCommands.containsKey(command)) {
      ...
      try {
        if (streams.output != null) {
          try {
            fileOut =
                new PrintStream(new java.io.FileOutputStream(streams.output, streams.appendOutput));
  ...
  /**
   * Tokenizes the input string, respecting quotes and escape characters.
   *
   * @param input The command line input string.
  ...
  /**
   * Parses redirection operators and separates command tokens from output/error redirection
   * targets.
   *
   * @param parts The list of command tokens.
   * @return An ExtractResult containing command tokens and redirection info.
   */
  private ExtractResult extractStreams(List<String> parts) {
    ...
    return new ExtractResult(newCommands, new Streams(output, err, appendOutput, appendErr));
  }

  /**
   * Executes a pipeline of commands, handling both builtins and external commands, and managing
   * inter-process communication.
   *
   * @param parts The list of pipeline command segments.
   * @param streams Redirection targets for output and error.
   * @param currentDirectory The current working directory.
   * @return The updated current directory after pipeline execution.
   */
  ...
  /**
   * Returns the appropriate Command instance (builtin or external) for the given tokens.
   *
  ...
  public Command getCommand(
      String[] tokens, String rawInput, File redirectFile, File stderrRedirectFile) {
    if (tokens.length == 0) {
  ...
  /**
   * Executes an external (non-builtin) command, handling output/error redirection.
   *
  ...
  private void handleExternalCommand(
      List<String> commands, Streams streams, Path currentDirectory) {
    try {
